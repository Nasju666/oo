...
// Fetch products with stock information for authenticated user only
$sql = "SELECT product_id, product_name, category, stock FROM tblproduct WHERE user_id = ? ORDER BY stock ASC";
                ...
                if ($result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<tr>
                            ...
                            <td>{$row['category']}</td>
                            <td>{$stock}</td>
                            <td style='background-color:{$color}; color:{$textColor}; font-weight:bold; text-align:center;'>{$alert}</td>
                          </tr>";
                    }
                } else {
